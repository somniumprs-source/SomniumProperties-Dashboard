 ...
 * lead 360 e P&L regional. Port de src/db/regiaoRoutes.js (Express -> Hono).
 * Registado sob basePath /crm em crm/index.ts. Os 13 endpoints só dependem
 * do pool — portados na íntegra.
 */
import pool from "../_shared/pg.ts";

const REGIOES = ["Coimbra", "AMP"];
const CONCELHOS_POR_REGIAO: Record<string, string[]> = {
...
export function registerRegiaoRoutes(app: any) {
  // ── Catálogo de regiões e concelhos ──────────────────────────
  app.get("/regiao/catalog", (c: any) => {
    return c.json({ regioes: REGIOES, concelhos: CONCELHOS_POR_REGIAO });
  });

  // ── KPIs por região (resumo executivo) ───────────────────────
  app.get("/regiao/kpis", async (c: any) => {
    try {
      const regiao = c.get("regiaoActiva") || c.req.query("regiao");
      if (!regiao) return c.json({ error: "X-Regiao em falta" }, 400);
  ...
  app.get("/regiao/mercado", async (c: any) => {
    try {
      const regiao = c.get("regiaoActiva") || c.req.query("regiao");
      const concelho = c.req.query("concelho");
  ...
  app.post("/regiao/mercado", async (c: any) => {
    try {
  ...
  app.put("/regiao/mercado/:id", async (c: any) => {
    try {
      const { regiao, concelho, freguesia, tipologia, eur_m2_compra, eur_m2_venda, tempo_medio_venda_dias, taxa_absorcao_pct, fonte, data_referencia, notas } = await c.req.json().catch(() => ({}));
      await pool.query(
        ...
        [c.req.param("id"), regiao, concelho, freguesia || null, tipologia || null, eur_m2_compra || null, eur_m2_venda || null, tempo_medio_venda_dias || null, taxa_absorcao_pct || null, fonte || null, data_referencia || null, notas || null]);
      return c.json({ ok: true });
    } catch (e) { return c.json({ error: (e as Error).message }, 400); }
  });

  app.delete("/regiao/mercado/:id", async (c: any) => {
    try {
      await pool.query(`DELETE FROM mercado_referencia WHERE id=$1`, [c.req.param("id")]);
      return c.json({ ok: true });
    } catch (e) { return c.json({ error: (e as Error).message }, 500); }
  });
  ...
  app.get("/regiao/match/imovel/:id", async (c: any) => {
    try {
      const imRes = await pool.query(`SELECT * FROM imoveis WHERE id=$1`, [c.req.param("id")]);
      if (!imRes.rows[0]) return c.json({ error: "Imóvel não encontrado" }, 404);
      ...
      const scored = invs.rows.map((inv: any) => {
        let score = 0;
        ...
        // Região (25 pts)
        let regs: any[] = [];
        try { regs = JSON.parse(inv.regioes_preferidas || "[]"); } catch { /* ignore */ }
  ...
  // ── Compliance regional ──────────────────────────────────────
  app.get("/regiao/compliance", async (c: any) => {
    try {
      const regiao = c.get("regiaoActiva") || c.req.query("regiao");
      const concelho = c.req.query("concelho");
  ...
  app.put("/regiao/compliance/:concelho", async (c: any) => {
    try {
      const { regiao, imt_perc_base, imi_perc, aimi_perc, zona_aru, notas_legais, contactos_uteis } = await c.req.json().catch(() => ({}));
      const id = `compl-${(c.req.param("concelho") || "").toLowerCase().replace(/\s+/g, "-")}`;
      await pool.query(
        ...
        [id, regiao || "Coimbra", c.req.param("concelho"), imt_perc_base || null, imi_perc || null, aimi_perc || null, zona_aru || false, notas_legais || null, contactos_uteis || null]);
      return c.json({ ok: true });
    } catch (e) { return c.json({ error: (e as Error).message }, 400); }
  });

  // ── Lead 360 — interacções unificadas ────────────────────────
  app.get("/regiao/lead-360/:entidade/:id", async (c: any) => {
    try {
  ...
  app.post("/regiao/lead-360", async (c: any) => {
    try {
      const id = crypto.randomUUID();
      const { entidade_tipo, entidade_id, canal, direcao, assunto, conteudo, regiao, utilizador, metadata } = await c.req.json().catch(() => ({}));
      if (!entidade_tipo || !entidade_id || !canal) return c.json({ error: "entidade_tipo, entidade_id e canal obrigatórios" }, 400);
      await pool.query(
        ...
        [id, entidade_tipo, entidade_id, canal, direcao || null, assunto || null, conteudo || null, regiao || c.get("regiaoActiva") || null, utilizador || null, metadata ? JSON.stringify(metadata) : "{}"]);
      return c.json({ id }, 201);
    } catch (e) { return c.json({ error: (e as Error).message }, 400); }
  });

  // ── Benchmarking de consultores na região ────────────────────
  app.get("/regiao/benchmarking/consultores", async (c: any) => {
    try {
      const regiao = c.get("regiaoActiva") || c.req.query("regiao");
      const { rows } = await pool.query(
        ...
        [regiao]);
      return c.json({ data: rows, total: rows.length, regiao });
    } catch (e) { return c.json({ error: (e as Error).message }, 500); }
  });

  // ── Hot zones — agregação por concelho (para mapa) ──────────
  app.get("/regiao/hot-zones", async (c: any) => {
    try {
      const regiao = c.get("regiaoActiva") || c.req.query("regiao");
      const { rows } = await pool.query(
        `SELECT
         ...
         ORDER BY total DESC`,
        [regiao]);
      return c.json({ regiao, data: rows });
    } catch (e) { return c.json({ error: (e as Error).message }, 500); }
  });
}
