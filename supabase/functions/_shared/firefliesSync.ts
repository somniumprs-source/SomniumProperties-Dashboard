/**
 * Integração Fireflies.ai → CRM. Port de src/db/firefliesSync.js.
 * Puxa transcrições de reuniões e guarda na base de dados.
 * Liga automaticamente a investidores/consultores pelo nome.
 *
 * Nota: o firefliesSync.js original NAO importa meetingAnalysis.js — apenas faz
 * fetch + persistencia das transcricoes. Por isso nao ha analise de reuniao a
 * stubbar aqui. (A analise via Anthropic vive em meetingAnalysis.js, nao portado,
 * e e chamada noutro fluxo.)
 */
import pool from "../_shared/pg.ts";

const FIREFLIES_API = "https://api.fireflies.ai/graphql";
const API_KEY = Deno.env.get("FIREFLIES_API_KEY") || "";
...
export async function syncFireflies() {
  ...
  for (const t of transcripts) {
    ...
    const { rows: [existing] } = await pool.query(
      "SELECT id FROM reunioes WHERE fireflies_id = $1",
      [t.id],
    );
    if (existing) {
      skipped++;
    ...
    const transcricao = (t.sentences || []).map((s: any) => `[${s.speaker_name}]: ${s.text}`).join("\n");

    // Resumo
    const resumo = t.summary?.overview || "";
    const keywords = Array.isArray(t.summary?.keywords)
 ...
 */
async function matchEntity(title: string, _participants: unknown) {
