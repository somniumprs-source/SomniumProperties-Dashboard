...
// (filesystem efémero das Edge Functions).
//
...
// Labels usados pelo footer dos PDFs — espelha DOC_LABELS em pdfImovelDocs.ts
...
async function uploadToStorage(storagePath, buf) {
  if (!supabase) throw new Error('Storage indisponivel')
  await ensureBucket()
...
export async function persistDocumento(imovel, tipo, opts = {}) {
  const { trigger, generatedBy = 'system', frozen = false, analise = null } = opts
