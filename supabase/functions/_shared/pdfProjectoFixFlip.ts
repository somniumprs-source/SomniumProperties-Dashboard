 ...
 * Port de src/db/pdfProjectoFixFlip.js (Express -> Edge Functions / Deno).
 * Diferenças: logo via LOGO_WHITE_PNG (sem leitura de disco; variante branca
 * porque o header é preenchido a preto) e fotos via buffers `_data`
 * pré-carregados (preload por fetch nos handlers) em vez de paths de disco —
 * o `/public/uploads` não existe nos isolates.
 */
...
function header(doc, titulo, subtitulo) {
  ...
  try { doc.image(LOGO_WHITE_PNG, 50, 30, { height: 40 }) } catch {}
...
// As fotos chegam com `_data` (Buffer) pré-carregado pelos handlers (preload
// por fetch das URLs Supabase). Sem `_data`, a foto é ignorada (sem disco).
function inserirFotos(doc, fotos, maxFotos = 6) {
  ...
  for (let i = 0; i < fotosShow.length; i++) {
    ...
    const f = fotosShow[i]
    const imgData = f._data || null
    if (imgData) {
      try {
        doc.save()
        doc.roundedRect(x, doc.y, cellW, cellH, 4).clip()
        doc.image(imgData, x, doc.y, { fit: [cellW, cellH], align: 'center', valign: 'center' })
        doc.restore()
