 ...
 * Port de src/db/projetoAuditLog.js (Express -> Edge Functions).
 */
import pool from './pg.ts'

const randomUUID = () => crypto.randomUUID()

export async function audit(opts: any) {
  const { negocioId, entidade, entidadeId, acao, campo, valorAntes, valorDepois, descricao, user } = opts || {}
  try {
    await pool.query(
