 ...
 * Port de src/db/sopDriveImport.js para Deno/Edge Functions.
 *
 ...
 *
 * Adaptação: usa getGoogleAuth() ENV-ONLY (sem disco) e o cliente @googleapis/drive,
 * o mesmo padrão já provado em ./driveSync.ts.
 */
...
export async function importFolderToSops({ folderId, departamento, overwrite = false, user = null }) {
  if (!isConfigured()) {
    throw new Error("Google Drive não está configurado. Definir GOOGLE_CLIENT_ID/SECRET/REFRESH_TOKEN nos secrets da função.");
  }
  if (!DEPARTAMENTOS_VALIDOS.includes(departamento)) {
    throw new Error(`Departamento inválido: ${departamento}`);
  }
  const d = getDrive();
  if (!d) throw new Error("Não foi possível instanciar o cliente Drive.");
 ...
 *     campo é gerido pela reclassificação manual no CRM (endpoint PUT
