 ...
 *
 * Port verbatim de src/db/orcamentoObraEngine.js — fórmulas inalteradas.
 */
...
function taxaMaterial(zonaAru: boolean): number { return zonaAru ? 6 : 23 }
function taxaMO(tipoObra: string): number { return tipoObra === 'construcao_nova' ? 23 : 6 }

// ── Construtor de linha ────────────────────────────────────
function linha({ descricao, base, taxa_iva = 23, autoliquidacao = false, retencao_irs = 0, capitalizavel = true, formula, tipo = 'misto' }: any): any {
...
function linhaMaterial(descricao: string, base: number, formula: string, tMat: number): any {
...
function linhaMODirecta(descricao: string, base: number, formula: string, tMO: number, autoliq: boolean): any {
  if (num(base) <= 0) return null
  return linha({
...
function linhaServico(descricao: string, base: number, formula: string, tMO: number, autoliq: boolean): any {
  if (num(base) <= 0) return null
  return linha({
    descricao, base, taxa_iva: tMO,
    autoliquidacao: !!autoliq,
...
function linhaHonorarios(descricao: string, base: number, formula: string, retencao = 0): any {
  if (num(base) <= 0) return null
  return linha({ descricao, base, taxa_iva: 23, retencao_irs: retencao, formula, tipo: 'honorarios' })
}

function linhaTaxa(descricao: string, base: number, formula?: string): any {
  if (num(base) <= 0) return null
  return linha({ descricao, base, taxa_iva: 0, formula: formula || `${num(base)} € (sem IVA)`, tipo: 'taxas' })
}

function linhaIsento(descricao: string, base: number, formula?: string): any {
  if (num(base) <= 0) return null
...
// 1. Demolições
function linhasDemolicoes(s: any, regime: any): any[] {
  s = s || {}
...
// 2. Estaleiro de obra
function linhasEstaleiro(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 3. RCD — DL 102-D/2020
function linhasRCD(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const out = []
...
// 4. Estrutura
function linhasEstrutura(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
function linhasEletricidade(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 6. AVAC / Solar / AQS
function linhasAvac(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
  const items: any[] = [
    ['split_un', 'split_eur_un', 'Splits / multi-split AC', (un: any, eur: any) => `${un} × ${eur} €/un`],
    [null, 'bomba_calor', 'Bomba de calor (AQS)', (v: any) => `${v} €`],
    [null, 'solar_termico', 'Painéis solares térmicos + dep.', (v: any) => `${v} €`],
    ['fotovoltaico_kwp', 'fotovoltaico_eur_kwp', 'Sistema fotovoltaico', (k: any, eur: any) => `${k} kWp × ${eur} €/kWp`],
    [null, 'recuperador', 'Recuperador / lareira', (v: any) => `${v} €`],
    [null, 'outros_avac', 'Outros equipamentos AVAC', (v: any) => `${v} €`],
  ]
...
// 7. Pavimento
function linhasPavimento(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 8. Pladur tetos
function linhasPladur(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
// 9. Isolamento
function linhasIsolamento(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
function linhasImpermeabilizacoes(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
function linhasRebocos(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
// 12. Caixilharias
function linhasCaixilharias(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 13. VMC
function linhasVmc(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
// 14. Pintura
function linhasPintura(s: any, pisos: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 15. Casas de banho
function linhasCasasBanho(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const auto = !!s.autoliquidacao
...
// 16. Portas
function linhasPortas(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const un = num(s.un)
...
function linhasCarpintarias(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
// 18. Serralharias
function linhasSerralharias(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
// 19. Cozinhas
function linhasCozinhas(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const un = num(s.un)
...
function linhasCapoto(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
function linhasAndaimes(s: any, regime: any): any[] {
  s = s || {}
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
function linhasCobertura(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
...
function linhasEquipamento(s: any, regime: any): any[] {
  s = s || {}
  const tMat = taxaMaterial(regime.zona_aru)
  const out = []
  for (const [k, label] of [
...
function linhasLicenciamento(s: any): any[] {
  s = s || {}
  const out = []
  if (num(s.projeto) > 0)
...
function linhasFechoObra(s: any, regime: any): any[] {
  s = s || {}
  const tMO = taxaMO(regime.tipo_obra)
  const out = []
  // Honorários
  ...
  if (num(s.limpeza_dias) > 0 && num(s.limpeza_eur_dia) > 0)
    out.push(linhaServico('Limpeza final pós-obra',
      ...
      `${num(s.limpeza_dias)} dias × ${num(s.limpeza_eur_dia)} €/dia`, tMO, false))
  return out.filter(Boolean)
}

// ── Linhas livres (custom) ─────────────────────────────────
// Tipo derivado pelo utilizador (ou auto: se unidade=dias → MO; senão → material)
function linhasCustom(s: any, regime: any): any[] {
...
function comCustom(resolverFn: any): any {
  return (s: any, p: any, r: any) => [...(resolverFn(s, p, r) || []), ...linhasCustom(s, r)]
}
...
const RESOLVERS: Record<string, any> = {
  estaleiro:           comCustom((s: any, p: any, r: any) => linhasEstaleiro(s, r)),
  demolicoes:          comCustom((s: any, p: any, r: any) => linhasDemolicoes(s, r)),
  rcd:                 comCustom((s: any, p: any, r: any) => linhasRCD(s, r)),
  estrutura:           comCustom((s: any, p: any, r: any) => linhasEstrutura(s, r)),
  eletricidade:        comCustom((s: any, p: any, r: any) => linhasEletricidade(s, p, r)),
  avac:                comCustom((s: any, p: any, r: any) => linhasAvac(s, r)),
  pavimento:           comCustom((s: any, p: any, r: any) => linhasPavimento(s, p, r)),
  pladur:              comCustom((s: any, p: any, r: any) => linhasPladur(s, p, r)),
  isolamento:          comCustom((s: any, p: any, r: any) => linhasIsolamento(s, r)),
  impermeabilizacoes:  comCustom((s: any, p: any, r: any) => linhasImpermeabilizacoes(s, r)),
  rebocos:             comCustom((s: any, p: any, r: any) => linhasRebocos(s, r)),
  caixilharias:        comCustom((s: any, p: any, r: any) => linhasCaixilharias(s, p, r)),
  vmc:                 comCustom((s: any, p: any, r: any) => linhasVmc(s, p, r)),
  pintura:             comCustom((s: any, p: any, r: any) => linhasPintura(s, p, r)),
  casas_banho:         comCustom((s: any, p: any, r: any) => linhasCasasBanho(s, r)),
  portas:              comCustom((s: any, p: any, r: any) => linhasPortas(s, r)),
  carpintarias:        comCustom((s: any, p: any, r: any) => linhasCarpintarias(s, r)),
  serralharias:        comCustom((s: any, p: any, r: any) => linhasSerralharias(s, r)),
  cozinhas:            comCustom((s: any, p: any, r: any) => linhasCozinhas(s, r)),
  andaimes:            comCustom((s: any, p: any, r: any) => linhasAndaimes(s, r)),
  capoto:              comCustom((s: any, p: any, r: any) => linhasCapoto(s, r)),
  cobertura:           comCustom((s: any, p: any, r: any) => linhasCobertura(s, r)),
  equipamento:         comCustom((s: any, p: any, r: any) => linhasEquipamento(s, r)),
  licenciamento:       comCustom((s: any, p: any, r: any) => linhasLicenciamento(s)),
  fecho_obra:          comCustom((s: any, p: any, r: any) => linhasFechoObra(s, r)),
}
...
export const SECCOES_LABELS: Record<string, string> = {
...
function migrarRegime(regime: string): { zona_aru: boolean; tipo_obra: string } {
...
// Motor principal
// ════════════════════════════════════════════════════════════
export function calcOrcamentoObra(orcamento: any): any {
  const o = orcamento || {}
  ...
  const seccoes: Record<string, any> = {}
  ...
  for (const key of [...SECCOES_OBRA, ...SECCOES_EXTRA]) {
    ...
    const subtotal_base = round2(linhas.reduce((a: number, l: any) => a + l.base, 0))
    const subtotal_iva = round2(linhas.reduce((a: number, l: any) => a + (l.autoliquidacao ? 0 : l.iva), 0))
  ...
  const baseObra = round2(Object.entries(seccoes)
    ...
    .reduce((a: number, [, v]: any) => a + v.subtotal_base, 0))
  ...
  const total_iva_obra = round2(Object.entries(seccoes)
    .filter(([k]) => SECCOES_OBRA.includes(k))
    .reduce((a: number, [, v]: any) => a + v.subtotal_iva, 0) + bdi_iva)
  const total_iva_autoliq_obra = round2(Object.entries(seccoes)
    .filter(([k]) => SECCOES_OBRA.includes(k))
    .reduce((a: number, [, v]: any) => a + v.iva_autoliq, 0))
  const total_retencoes_obra = round2(Object.entries(seccoes)
    .filter(([k]) => SECCOES_OBRA.includes(k))
    .reduce((a: number, [, v]: any) => a + v.retencoes, 0))
  ...
  const ivaExtra = round2(Object.entries(seccoes)
    .filter(([k]) => SECCOES_EXTRA.includes(k))
    .reduce((a: number, [, v]: any) => a + v.subtotal_iva, 0))
  ...
  return {
    ...
    subtotais: Object.fromEntries(Object.entries(seccoes).map(([k, v]: any) => [k, v.subtotal_bruto])),
  }
}

// ── Validação fiscal + aritmética ──────────────────────────
export function validarOrcamento(orcamento: any): any[] {
  const avisos = []
  const o = orcamento || {}
