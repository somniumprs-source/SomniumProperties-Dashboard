 ...
 * Port de src/db/whatsappAgent.js para Deno/Edge Functions.
 *
 * Diferencas para Deno/Edge (sem alterar a logica do agente):
 *  - process.env -> Deno.env.get; randomUUID -> crypto.randomUUID().
 *  - validateTwilioSignature reescrita com Web Crypto (HMAC-SHA1 async).
 *  - sendWhatsApp via REST directo (sem SDK twilio).
 *  - Queue/setTimeout (messageBuffers) REMOVIDA: cada mensagem inbound e
 *    processada de imediato por handleInboundMessage (Edge e stateless e o
 *    isolate pode morrer entre pedidos, portanto buffers em memoria nao servem).
 *  - notificationService (googleapis+OAuth) NAO portado: stub local diferido.
 */
import pool from "./pg.ts";
import { detectPortalLink, downloadPortalPhotos, fetchPortalData } from "./portalFetch.ts";
import { sendEscalacaoEmail } from "./emailService.ts";
...
// Stub local: a notificacao secundaria por email (originalmente em
// notificationService.js via googleapis+OAuth) e diferida para nao puxar OAuth
// de ficheiro para a Edge Function. Mantida fire-and-forget, sem efeito.
async function sendWhatsAppNotification(_: any): Promise<void> {
  /* notificacao secundaria diferida — ver comentario acima */
}
...
async function withRetry<T>(fn: () => Promise<T>, label = "op"): Promise<T> {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const isRetryable = e.status === 429 || e.status === 500 || e.status === 502 ||
        e.status === 503 || e.code === "ECONNRESET";
      if (!isRetryable || attempt === MAX_RETRIES) throw e;
      const delay = Math.min(1000 * Math.pow(2, attempt - 1), 8000);
      console.warn(
        `[agent] ${label} tentativa ${attempt}/${MAX_RETRIES} falhou (${e.status || e.code}), retry em ${delay}ms`,
      );
      await new Promise((r) => setTimeout(r, delay));
    }
  }
  throw new Error(`[agent] ${label} esgotou retries`);
}

// ── Validacao de assinatura Twilio (Web Crypto, HMAC-SHA1 base64) ──
export async function validateTwilioSignature(
  ...
  // Construir string: URL + params ordenados por chave (igual ao original)
...
// ── Enviar WhatsApp via Twilio REST (com retry) ─────────────
export async function sendWhatsApp(to: string, body: string): Promise<{ ok: boolean; sid?: string; error?: string }> {
  ...
  try {
    const sid = await withRetry(async () => {
      const auth = btoa(`${TWILIO_SID}:${TWILIO_TOKEN}`);
      const form = new URLSearchParams();
      form.set("From", `whatsapp:${TWILIO_NUMBER}`);
      ...
      if (!resp.ok) {
        const err: any = new Error(json?.message || `Twilio HTTP ${resp.status}`);
        err.status = resp.status;
        throw err;
      }
      return json.sid as string;
    }, "sendWhatsApp");
    console.log("[whatsapp] Enviado:", sid, "→", to);
    return { ok: true, sid };
  } catch (e: any) {
...
const ZONAS_INTERESSE_TEXT: Record<string, string> = {
...
const RESPONDER_CRITERIOS_TEXT: Record<string, string> = {
  Coimbra:
    "Procuramos imóveis com margem de negociação — construção antiga ou que precise de obras, onde haja espaço para criar valor. Zonas de Coimbra, Condeixa e arredores. Até 250k.",
  AMP:
    "Procuramos imóveis com margem de negociação — construção antiga ou que precise de obras, onde haja espaço para criar valor. Zonas: Porto, Vila Nova de Gaia e Santa Maria da Feira. Até 250k.",
};
const RESPONDER_QUEM_SOMOS_TEXT: Record<string, string> = {
  Coimbra:
  ...
  AMP:
    "Somos a Somnium Properties — investimos em imóveis com potencial na Área Metropolitana do Porto (Porto, Gaia, Santa Maria da Feira). Compramos, renovamos e colocamos novamente no mercado. Trabalhamos com consultores para encontrar as melhores oportunidades.",
};

// Gera o system prompt parametrizado pela região do consultor. Default Coimbra.
export function buildSystemPrompt(regiao = "Coimbra"): string {
...
const VALID_DECISOES = [
  "ADICIONAR", "TRIAGEM", "IGNORAR", "RESPONDER_CRITERIOS",
  "RESPONDER_QUEM_SOMOS", "AGUARDAR", "DUPLICADO", "ESCALAR",
];

function validateDecision(raw: any): any {
  ...
  return {
    ...
    imovel: raw.imovel && typeof raw.imovel === "object"
      ? {
        ...
        ask_price: typeof raw.imovel.ask_price === "number"
          ? raw.imovel.ask_price
          : parseFloat(raw.imovel.ask_price) || null,
        area_m2: typeof raw.imovel.area_m2 === "number"
          ? raw.imovel.area_m2
          : parseFloat(raw.imovel.area_m2) || null,
...
function extractJson(text: string): any {
  ...
  for (let i = 0; i < text.length; i++) {
    if (text[i] === "{") {
      ...
      if (depth === 0 && start >= 0) {
        try {
          lastValid = JSON.parse(text.slice(start, i + 1));
        } catch { /* continuar */ }
        start = -1;
      }
    }
...
// ── Processar uma mensagem unica (logica do antigo processMessages, sem buffer) ──
// messages: lista de { body, timestamp } — mantida como lista para preservar
// o comportamento de "combinedText" do original (aqui sera tipicamente 1 mensagem).
async function processSingle(phone: string, messages: { body: string; timestamp: string }[]): Promise<void> {
  if (!messages || messages.length === 0) return;

  const combinedText = messages.map((m) => m.body).join("\n");
  const urgente = messages.some((m) => isUrgent(m.body));
  const now = new Date();
  let consultor: any = null;

  console.log(`[agent] Processando ${messages.length} msgs de ${phone} (urgente: ${urgente})`);
  ...
  try {
    ...
    await pool.query(
      ...
      [crypto.randomUUID(), consultor.id, now.toISOString(), "whatsapp", "Recebido", combinedText],
    );
    ...
    }).catch((e: any) => console.warn("[notif] Erro notificacao recebida:", e.message));
    ...
    const historicoText = historico.reverse().map((h: any) =>
      `[${new Date(h.data_hora).toLocaleString("pt-PT")}] ${
        h.direcao === "Enviado" ? "Alexandre" : consultor.nome
      }: ${h.notas}`
    ).join("\n");

    // 7. Carregar imoveis recentes para deteccao de duplicados (ultimos 90 dias + activos)
    const cutoff90d = new Date(now.getTime() - 90 * 86400000).toISOString();
...
AGÊNCIA: ${(() => {
      try {
        return JSON.parse(consultor.imobiliaria || "[]").join(", ");
      } catch {
        return "Desconhecida";
      }
    })()}
...
${
      portalData
        ? `DADOS EXTRAÍDOS DO PORTAL (${portalLink!.portal}):
Link: ${portalLink!.url}
...
${imoveisExistentes.map((i: any) => `- ${i.nome} | ${i.zona || "?"} | ${i.tipologia || "?"} | ${i.ask_price || "?"}€`).join("\n")}

MENSAGEM${messages.length > 1 ? "S" : ""} DO CONSULTOR:
${combinedText}
...
`;
    ...
    if (!decision) {
      ...
      await pool.query(
        ...
        [
          ...
          "whatsapp",
          "Enviado",
          `[ERRO AGENTE] Resposta Claude invalida — mensagem nao processada automaticamente`,
        ],
      );
        ...
        if (sent.ok) {
          await pool.query(
            ...
            [
              ...
              "whatsapp",
              "Enviado",
              `[AGENTE FALLBACK] ${fallbackMatch[1]}`,
            ],
          );
        }
      }
    ...
    console.log(
      `[agent] Decisao: ${decision.decisao} | Prioridade: ${decision.prioridade} | Confianca: ${decision.confianca}`,
    );
    ...
    if (decision.resposta_consultor) {
      ...
      if (sent.ok) {
        ...
        }).catch((e: any) => console.warn("[notif] Erro notificacao enviada:", e.message));
      }
    }
    ...
    if (decision.decisao === "ADICIONAR" && decision.imovel) {
      ...
      const isDuplicate = imoveisExistentes.some((e: any) => {
        ...
        const precoSimilar = im.ask_price && e.ask_price &&
          Math.abs(e.ask_price - im.ask_price) / e.ask_price <= 0.10;
      ...
      if (isDuplicate) {
        await sendWhatsApp(
          phone,
          "Esse imóvel já está no nosso radar — estamos a acompanhar a situação. Se houver novidade do lado do proprietário, avisa-nos!",
        );
    ...
    try {
      ...
      const temposResp: number[] = [];
      for (let i = 0; i < allInt.length; i++) {
        if (allInt[i].direcao === "Enviado") {
          const resp = allInt.slice(i + 1).find((x: any) => x.direcao === "Recebido" || x.direcao === "Resposta");
          if (resp) {
            const horas = (new Date(resp.data_hora).getTime() - new Date(allInt[i].data_hora).getTime()) / 3600000;
...
// ── Receber mensagem WhatsApp (inbound, sem buffer/timer) ────
// Substitui receiveWhatsAppMessage + processMessages: processa a mensagem de
// imediato. Em Edge Functions o isolate e efemero, por isso nao ha como manter
// buffers em memoria entre pedidos — cada inbound e auto-contido.
export async function handleInboundMessage(from: string, body: string, isFromAgent = true): Promise<void> {
  ...
  if (!isFromAgent) {
    ...
    ).catch((e: any) => console.warn("[agent] Erro handoff:", e.message));
    return;
  }

  await processSingle(phone, [{ body, timestamp: new Date().toISOString() }]);
}
