...
function pctOrDash(v: any): string {
  if (v == null || isNaN(parseFloat(v))) return PCT_DEFAULT
  return `${parseFloat(v)}%`
}

function regimeIvaLabel(deal: any): string {
...
function modoObraLabel(deal: any): string {
  if (deal.modo_obra === 'fixo') return 'Fixo (empreiteiro)'
  return 'Calculado (PMO + IVA)'
}

function financiamentoLabel(deal: any): string {
...
export function renderAssumptionsAndGlossary(b: any, deal: any, opts: any = {}): void {
  if (!b || !deal) return
