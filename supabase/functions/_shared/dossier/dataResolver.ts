...
import { calcAnalise } from '../calcEngine.ts'
import { computeMOIC, computePayback } from './metrics.ts'
...
const numOr = (v: any, fallback = 0): number => {
  const n = parseFloat(v)
...
function buildCalcInputs(imovel: any, analise: any): any {
  const im = imovel || {}
  const a = analise || {}
  const inputs: any = {}
...
function hasMinimumInputs(inputs: any): boolean {
...
export function resolveDealData(imovel: any, analise: any, _orcamento?: any): any {
  const im = imovel || {}
  const a = analise || {}
