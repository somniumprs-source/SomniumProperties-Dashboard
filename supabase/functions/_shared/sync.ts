// @ts-nocheck
/**
 * Sync bidirecional CRM <-> Notion. Port verbatim de src/db/sync.js.
 * Nunca apaga — só cria e atualiza.
 *
 * Adaptações para Deno/Edge Functions:
 *  - `import 'dotenv/config'` removido (env vem de Deno.env / secrets da função).
 *  - `process.env.NOTION_*` -> `Deno.env.get(...)`.
 *  - `randomUUID` de node:crypto -> `crypto.randomUUID()` (global no Deno).
 *  - Guards de NOTION_API_KEY: sem chave, syncToNotion / syncFromNotion fazem
 *    no-op gracioso (devolvem { skipped: 'notion-not-configured' }).
 */
...
const NOTION_DBS = {
  imoveis:      Deno.env.get("NOTION_DB_PIPELINE_IMOVEIS"),
  investidores: Deno.env.get("NOTION_DB_INVESTIDORES"),
  negocios:     Deno.env.get("NOTION_DB_FATURACAO"),
  despesas:     Deno.env.get("NOTION_DB_DESPESAS"),
  consultores:  Deno.env.get("NOTION_DB_CONSULTORES"),
  tarefas:      Deno.env.get("NOTION_DB_TAREFAS"),
};
...
export async function syncFromNotion(table) {
  if (!NOTION_API_KEY) return { table, skipped: 'notion-not-configured' }
...
export async function syncAllFromNotion() {
  if (!NOTION_API_KEY) return { skipped: 'notion-not-configured' }
  const results = {}
