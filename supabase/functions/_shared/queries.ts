...
function mapNegocio(r: any) {
  return {
    ...
    dataInicioObra: r.data_inicio_obra,
    dataFimObra: r.data_fim_obra,
    pagamentosFaseados: (() => {
      try {
...
export function isInvestidorPrincipal(i: any) {
...
export async function getProjetoInvestidores({ regiao }: { regiao?: string | null } = {}) {
...
export async function getReunioesInvestidor({ regiao }: { regiao?: string | null } = {}) {
  const clause = regiao ? "WHERE n.deleted_at IS NULL AND n.regiao = $1" : "WHERE n.deleted_at IS NULL";
  const params = regiao ? [regiao] : [];
  const { rows } = await pool.query(
    `SELECT r.* FROM reunioes_investidor r JOIN negocios n ON n.id = r.negocio_id ${clause}`, params);
  return rows.map((r: any) => ({ id: r.id, negocioId: r.negocio_id, dataHora: r.data_hora, estado: r.estado }));
}

// SOP 13, Passo 8 — vistorias semanais de obra, por projecto.
export async function getVistoriasObra({ regiao }: { regiao?: string | null } = {}) {
  const clause = regiao ? "WHERE n.deleted_at IS NULL AND n.regiao = $1" : "WHERE n.deleted_at IS NULL";
  const params = regiao ? [regiao] : [];
  const { rows } = await pool.query(
