...
function apresentacaoInvestidor(im, analise) {
  const content = [
    ...
    dataTable([
      ...
      { label: 'Tipologia', value: im.tipologia },
      { label: 'Estado', value: (im.estado || '').replace(/^\d+-/, '') },
      { label: 'Área', value: im.area_util ? `${im.area_util} m²` : '—' },
    ]),
...
const DOC_GENERATORS = {
  ...
  ficha_pre_visita: (im) => genericDoc('Ficha Pré-Visita', im),
  checklist_visita: (im) => genericDoc('Checklist de Visita', im),
  relatorio_visita: (im) => genericDoc('Relatório de Visita', im),
  proposta_formal: (im) => propostaFormal(im),
  apresentacao_investidor: (im, a) => apresentacaoInvestidor(im, a),
  resumo_negociacao: (im) => genericDoc('Resumo de Negociação', im),
  resumo_acordo: (im) => genericDoc('Resumo de Acordo', im),
  dossier_investimento: (im, a) => apresentacaoInvestidor(im, a),
  ficha_follow_up: (im) => genericDoc('Ficha Follow Up', im),
  ficha_cessao_posicao: (im) => genericDoc('Ficha Cedência de Posição', im),
  ficha_acompanhamento_obra: (im) => genericDoc('Ficha Acompanhamento de Obra', im),
