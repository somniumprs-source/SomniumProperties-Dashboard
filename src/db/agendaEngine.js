...
// ── 3b. Follow-up de imóvel agendado na hora ────────────────────────
// A aba "Follow Up" do imóvel (e a edição da ficha) chamam isto logo após
// gravar data_follow_up, sem esperar pelo "gerar-semana": cria — ou move,
// se já houver uma por fazer — a tarefa 'A fazer' marcada para esse dia
// (inicio = data sem hora → evento de dia inteiro no Google Calendar).
// Usa a mesma origem de ORIGEM_CAMPOS (imovel/data_follow_up) e data_limite
// = inicio, por isso gerarTarefasSinteticas reconhece-a e não duplica nem
...
const RANK_PRIORIDADE = { alta: 0, media: 1, baixa: 2 }
...
// Fila priorizada, em duas listas separadas (25/08/2026, a pedido do
// utilizador): "Catálogo" (template_id preenchido, editável na página) e
// "Automáticas" (cadeia de angariação + tarefas por entidade — geradas
// sozinhas pelo motor, mas continuam a precisar de escolha manual de
// bloco). Cadeias aparecem como um único item atómico (as duas ou
// nenhuma, nunca partidas).
export async function gerarFila(pool) {
  ...
  const filaEntidade = entidade.filter(t => !idsCadeia.has(t.id)).map(mapSimples)
