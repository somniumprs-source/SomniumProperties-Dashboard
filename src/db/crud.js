...
function createCRUD(table, { searchFields = ['nome'], defaultSort = 'created_at DESC' } = {}) {
  return {
    ...
    async create(rawData, { regiaoActiva = null, role = null } = {}) {
      ...
      await auditLog(table, id, 'INSERT', null, { id, ...cleanData }, regiaoActiva).catch(e => console.error('[auditLog]', table, id, e.message))
    ...
    async update(id, rawData, { regiaoActiva = null, role = null } = {}) {
      ...
      await auditLog(table, id, 'UPDATE', existing[0], cleanData, regiaoActiva).catch(e => console.error('[auditLog]', table, id, e.message))
    ...
    async delete(id, { regiaoActiva = null } = {}) {
      ...
      await auditLog(table, id, 'DELETE', existing[0], null, regiaoActiva).catch(e => console.error('[auditLog]', table, id, e.message))
      return true
    },
