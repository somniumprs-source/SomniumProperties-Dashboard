...
const GRAVACAO_PROMPT_CONTEXTOS = {
  cold_call: {
...
function buildGravacaoPrompt(consultorNome, tipoChamada) {
...
async function analisarTranscricaoIA(transcricao, consultorNome, tipoChamada) {
...
router.post('/gravacoes/:id/transcricao', async (req, res) => {
  try {
    ...
    try {
      ...
      const analise = await analisarTranscricaoIA(transcricao, cons?.nome, g.tipo_chamada)
      await pool.query(
...
router.post('/gravacoes/:id/analisar', async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE consultor_gravacoes SET estado = 'a_analisar', updated_at = $2 WHERE id = $1`,
      [req.params.id, new Date().toISOString()])
    const cons = await Consultores.getById(g.consultor_id)
    const analise = await analisarTranscricaoIA(g.transcricao, cons?.nome, g.tipo_chamada)
