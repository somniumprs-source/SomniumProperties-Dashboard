...
export function KpiCard({ icon: Icon, label, value, sub, tone = 'gray', size = 'md', onClick, className = '' }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-xl border border-gray-200 ${s.card} flex items-center gap-3 ${interactive ? 'cursor-pointer hover:border-gray-300 hover:shadow-sm transition' : ''} ${className}`}>
      {Icon && (
        <div className={`${s.icon} rounded-xl ${t.iconBg} ${t.iconText} flex items-center justify-center shrink-0`}>
          <Icon className={s.iconSize} />
        </div>
      )}
      <div className="min-w-0 flex-1">
        {label && <p className={`${s.label} uppercase tracking-wide text-gray-400 font-semibold truncate`}>{label}</p>}
        <p className={`${s.value} font-bold ${t.accent} leading-tight truncate`}>{value ?? '—'}</p>
