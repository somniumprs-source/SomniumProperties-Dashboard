...
allpts = [d for d in data["imoveis"] if d.get("lat") and d.get("lng")]
def _cm2(d): return d.get("zona_m2_ref") or d.get("valor_mercado_m2") or 0
VMIN = min(_cm2(d) for d in allpts)
VMAX = max(_cm2(d) for d in allpts)
...
pts.forEach(function(p){{
  ...
  if(SHOW_LABELS && p.ask){{ m.bindTooltip(Math.round(p.ask/1000)+'k €',{{permanent:true,direction:'top',className:'lbl',offset:[0,-6]}}); }}
...
LEGEND = (f'<div class="legend"><b>Mercado da zona (€/m²)</b>'
...
def render(path, pts, W, H, R, FS, PAD, zoom1, legend=False, zoomplus=False, show_labels=True):
    markers = json.dumps([{"lat": d["lat"], "lng": d["lng"], "m2": _cm2(d),
                           "ask": d.get("ask_price")} for d in pts])
    html = TPL.format(W=W, H=H, R=R, FS=FS, PAD=PAD, ZOOM1=zoom1, VMIN=VMIN, VMAX=VMAX,
                      MARKERS=markers, SHOW_LABELS=("true" if show_labels else "false"),
                      LEGEND_CSS=(LEGEND_CSS if legend else ""),
                      LEGEND=(LEGEND if legend else ""),
                      ZOOMPLUS=("if(map.getZoom()<13){map.setZoom(map.getZoom()+1);}" if zoomplus else ""))
    open(path, "w", encoding="utf-8").write(html)

# mapa geral — sem etiquetas (orientacao + cor; evita sobreposicao no centro de Gaia)
render(os.path.join(HERE, "map.html"), allpts, 1240, 1040, 13, 13, 40, 14, legend=True, zoomplus=True, show_labels=False)
print("map.html:", len(allpts), "marcadores")

# mini-mapas por zona — etiqueta = preco pedido (k€), distinto por imovel
zonas = {}
for d in allpts:
    zonas.setdefault(d["freguesia"] or "sem-zona", []).append(d)
for z, arr in zonas.items():
    render(os.path.join(HERE, f"map_{slug(z)}.html"), arr, 1000, 560, 15, 14, 60, 15, show_labels=True)
