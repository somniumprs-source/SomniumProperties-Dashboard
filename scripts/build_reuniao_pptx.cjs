...
// Tema claro (creme/branco) com acento ouro; capa e fecho em escuro (sandwich).
...
const C = {
  page: "F6F4EE",       // fundo creme dos slides de conteudo
  ink: "1C1B18",        // titulos / texto forte
  body: "55514A",       // corpo
  muted: "8C887C",      // rotulos pequenos neutros
  gold: "C9A84C",       // barras, tabs, circulos, preenchimentos
  goldTx: "A8821F",     // ouro para TEXTO em fundo claro (legivel)
  card: "FFFFFF",
  card2: "FBF9F3",
...
function base(slide, { eyebrow, title, num }) {
  ...
  if (eyebrow)
    slide.addText(eyebrow.toUpperCase(), {
      x: M, y: 0.42, w: W - 2 * M, h: 0.3, fontFace: FB, fontSize: 11, bold: true, color: C.goldTx, charSpacing: 3 });
  if (title)
    slide.addText(title, {
      x: M, y: 0.7, w: W - 2 * M, h: 0.85, fontFace: FH, fontSize: 30, bold: true, color: C.ink, lineSpacing: 32 });
...
(() => {
  ...
  s.addText("RELATÓRIO PARA REUNIÃO", { x: 0, y: 3.6, w: W, h: 0.35, align: "center", fontFace: FB, fontSize: 13, bold: true, color: C.gold, charSpacing: 4 });
  s.addText("Reunião Semanal", { x: 0, y: 4.0, w: W, h: 1.0, align: "center", fontFace: FH, fontSize: 46, bold: true, color: C.dwhite });
  s.addText("07/06/2026  ·  22h00", { x: 0, y: 5.1, w: W, h: 0.45, align: "center", fontFace: FH, fontSize: 20, color: C.gold });
  s.addText("Semana W23  ·  1 a 7 de Junho  ·  CEO, Investidores, Imóveis e Claude Code", { x: 0, y: 5.6, w: W, h: 0.35, align: "center", fontFace: FB, fontSize: 12, color: C.dsub });
})();
...
(() => {
  ...
  stats.forEach(([v, l], i) => {
    ...
    s.addText(v, { x, y: y + 0.2, w: cw, h: 0.85, align: "center", fontFace: FH, fontSize: 46, bold: true, color: C.goldTx });
  ...
  card(s, M + half + gap, cy, half, ch, C.tint);
  s.addText("META ATÉ AO FIM DO ANO", { x: M + half + gap + 0.35, y: cy + 0.3, w: half - 0.7, h: 0.3, fontFace: FB, fontSize: 12, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText("100 – 200 mil €", { x: M + half + gap + 0.35, y: cy + 0.7, w: half - 0.7, h: 0.9, fontFace: FH, fontSize: 44, bold: true, color: C.goldTx });
  s.addText("2 empresas · ~10 cedências · 4 CAEP. Pipeline e estrutura suportam.", { x: M + half + gap + 0.35, y: cy + 1.68, w: half - 0.7, h: 0.5, fontFace: FB, fontSize: 12.5, color: C.body });
})();
...
(() => {
  ...
  items.forEach(([n, t, d], i) => {
    ...
    s.addText(n, { x: x + 0.35, y: y + 0.3, w: cw - 0.7, h: 0.9, fontFace: FH, fontSize: 38, bold: true, color: C.goldTx });
    s.addText(t, { x: x + 0.35, y: y + 1.25, w: cw - 0.7, h: 0.7, fontFace: FH, fontSize: 19, bold: true, color: C.ink });
...
(() => {
  ...
  s.addText("ESTRUTURA DESENHADA", { x: M + 0.35, y: 2.08, w: colW - 0.7, h: 0.35, fontFace: FB, fontSize: 12, bold: true, color: C.goldTx, charSpacing: 1 });
  ...
  blocks.forEach(([t, d], i) => {
    ...
    s.addText(t, { x: M + 0.6, y: y + 0.15, w: colW - 1.0, h: 0.4, fontFace: FH, fontSize: 17, bold: true, color: C.ink });
    s.addText(d, { x: M + 0.6, y: y + 0.54, w: colW - 1.0, h: 0.35, fontFace: FB, fontSize: 13, color: C.body });
  });
  const rx = M + colW + 0.4;
  card(s, rx, 1.85, colW, 4.45, C.tint);
  s.addText("A FECHAR HOJE", { x: rx + 0.35, y: 2.08, w: colW - 0.7, h: 0.35, fontFace: FB, fontSize: 12, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText("1 empresa (com acordo parassocial) ou 3 entidades?", { x: rx + 0.35, y: 2.45, w: colW - 0.7, h: 0.8, fontFace: FH, fontSize: 20, bold: true, color: C.ink, lineSpacing: 24 });
  s.addText(bullets([
  ...
  ]), { x: rx + 0.35, y: 3.4, w: colW - 0.7, h: 2.7, fontFace: FB, fontSize: 14, color: C.body, lineSpacing: 22, paraSpaceAfter: 10 });
})();
...
(() => {
  ...
  items.forEach(([t, v, d], i) => {
    ...
    card(s, x, y, cw, ch); goldTab(s, x, y, cw);
    s.addText(t, { x: x + 0.35, y: y + 0.25, w: cw - 0.7, h: 0.4, fontFace: FH, fontSize: 17, bold: true, color: C.ink });
    s.addText(v, { x: x + 0.35, y: y + 0.66, w: cw - 0.7, h: 0.45, fontFace: FH, fontSize: 20, bold: true, color: C.goldTx });
...
(() => {
  ...
  s.addText("PLANO", { x: M + 0.35, y: 4.8, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText("Apresentar a 2-3 investidores reais já prontos. Controlamos a quem cedemos; com 20k faturados não há pressa.", { x: M + 0.35, y: 5.14, w: colW - 0.7, h: 1.05, fontFace: FB, fontSize: 13, color: C.body, lineSpacing: 18 });
  const rx = M + colW + 0.4;
  card(s, rx, 1.85, colW, 4.45, C.tint);
  s.addText("“Ou leva 5 de 30,\nou 0 de 0.”", { x: rx + 0.4, y: 2.85, w: colW - 0.8, h: 1.8, fontFace: FH, fontSize: 34, bold: true, italic: true, color: C.goldTx, align: "center", lineSpacing: 42 });
  s.addText("Posição firme: o intermediário nunca pode ganhar mais do que cada sócio.", { x: rx + 0.4, y: 4.75, w: colW - 0.8, h: 1.2, fontFace: FB, fontSize: 14, color: C.body, align: "center", lineSpacing: 20 });
})();
...
(() => {
  ...
  const colW = (W - 2 * M - 0.4) / 2;
  card(s, M, 1.85, colW, 4.45);
  s.addText("PORQUÊ", { x: M + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText(bullets([
  ...
  ]), { x: M + 0.35, y: 2.5, w: colW - 0.7, h: 3.6, fontFace: FB, fontSize: 14.5, color: C.body, lineSpacing: 22, paraSpaceAfter: 14 });
  const rx = M + colW + 0.4;
  card(s, rx, 1.85, colW, 4.45);
  s.addText("IMÓVEIS IDENTIFICADOS", { x: rx + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  const props = [
  ...
  props.forEach(([t, d], i) => {
    const y = 2.55 + i * 0.9;
    s.addText(t, { x: rx + 0.35, y, w: colW - 0.7, h: 0.35, fontFace: FH, fontSize: 14.5, bold: true, color: C.ink });
...
(() => {
  ...
  rows.forEach(([t, d], i) => {
    ...
    s.addText(t, { x: M + 0.3, y, w: 3.0, h: rh, fontFace: FH, fontSize: 16, bold: true, color: C.goldTx, valign: "middle" });
    s.addText(d, { x: M + 3.4, y, w: W - 2 * M - 3.7, h: rh, fontFace: FB, fontSize: 13.5, color: C.body, valign: "middle" });
    y += rh + gap;
  });
...
(() => {
  ...
  const colW = (W - 2 * M - 0.4) / 2;
  card(s, M, 1.85, colW, 4.45);
  s.addText("A IDEIA", { x: M + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText(bullets([
  ...
  ]), { x: M + 0.35, y: 2.5, w: colW - 0.7, h: 3.6, fontFace: FB, fontSize: 14, color: C.body, lineSpacing: 21, paraSpaceAfter: 13 });
  const rx = M + colW + 0.4;
  card(s, rx, 1.85, colW, 4.45, C.tint);
  s.addText("DISCIPLINA", { x: rx + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText(bullets([
  ...
  ]), { x: rx + 0.35, y: 2.5, w: colW - 0.7, h: 3.6, fontFace: FB, fontSize: 14, color: C.body, lineSpacing: 21, paraSpaceAfter: 13 });
})();
...
(() => {
  ...
  items.forEach(([t, d], i) => {
    ...
    card(s, x, y, cw, 1.38);
    s.addText(t, { x: x + 0.32, y: y + 0.2, w: cw - 0.64, h: 0.4, fontFace: FH, fontSize: 16, bold: true, color: C.goldTx });
  ...
  s.addText("Purga automática", { x: M + 0.32, y: yb + 0.2, w: W - 2 * M - 0.64, h: 0.4, fontFace: FH, fontSize: 16, bold: true, color: C.goldTx });
...
(() => {
  ...
  const colW = (W - 2 * M - 0.4) / 2;
  card(s, M, 1.85, colW, 4.45);
  s.addText("A TENSÃO E A REGRA", { x: M + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText("Gerar documentos à pressa, sem ler nem validar, já se nota à distância e tira credibilidade.", { x: M + 0.35, y: 2.45, w: colW - 0.7, h: 1.1, fontFace: FB, fontSize: 14.5, color: C.body, lineSpacing: 20 });
  s.addShape("roundRect", { x: M + 0.35, y: 3.95, w: colW - 0.7, h: 2.0, rectRadius: 0.05, fill: { color: C.tint }, line: { color: C.gold, width: 1 } });
  s.addText("REGRA", { x: M + 0.6, y: 4.18, w: colW - 1.0, h: 0.3, fontFace: FB, fontSize: 11, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText("Só partilhar versões finais: lidas, corrigidas e validadas. Nada de versões intermédias.", { x: M + 0.6, y: 4.5, w: colW - 1.0, h: 1.3, fontFace: FH, fontSize: 17, bold: true, color: C.ink, lineSpacing: 23 });
  const rx = M + colW + 0.4;
  card(s, rx, 1.85, colW, 4.45);
  s.addText("LANDING DE INVESTIR", { x: rx + 0.35, y: 2.08, w: colW - 0.7, h: 0.3, fontFace: FB, fontSize: 11.5, bold: true, color: C.goldTx, charSpacing: 1 });
  s.addText(bullets([
  ...
  ]), { x: rx + 0.35, y: 2.5, w: colW - 0.7, h: 3.4, fontFace: FB, fontSize: 14.5, color: C.body, lineSpacing: 22, paraSpaceAfter: 14 });
})();
...
(() => {
  ...
  items.forEach(([t, dsc], i) => {
    ...
    s.addText(String(i + 1), { x: M + 0.3, y: cyc, w: d, h: d, align: "center", valign: "middle", fontFace: FH, fontSize: 18, bold: true, color: C.dwhite });
    s.addText(t, { x: M + 1.05, y, w: 2.6, h: rh, fontFace: FH, fontSize: 16, bold: true, color: C.ink, valign: "middle" });
    s.addText(dsc, { x: M + 3.65, y, w: W - 2 * M - 3.95, h: rh, fontFace: FB, fontSize: 13.5, color: C.body, valign: "middle" });
    y += rh + 0.13;
  });
})();
...
(() => {
  ...
  items.forEach(([p, t, d], i) => {
    ...
    card(s, x, y, cw, ch); goldTab(s, x, y, cw);
    s.addText(p, { x: x + 0.3, y: y + 0.22, w: 1.2, h: 0.5, fontFace: FH, fontSize: 24, bold: true, color: C.goldTx });
    s.addText(t, { x: x + 0.3, y: y + 0.8, w: cw - 0.6, h: 0.5, fontFace: FH, fontSize: 15.5, bold: true, color: C.ink });
...
(() => {
  ...
  items.forEach((t, i) => {
    card(s, M, y, W - 2 * M, rh, i % 2 ? C.card2 : C.card);
    s.addShape("rect", { x: M + 0.32, y: y + (rh - bx) / 2, w: bx, h: bx, fill: { color: C.page }, line: { color: C.gold, width: 1.5 } });
    s.addText(t, { x: M + 0.95, y, w: W - 2 * M - 1.2, h: rh, fontFace: FB, fontSize: 15, color: C.ink, valign: "middle" });
    y += rh + 0.14;
  });
})();

// ============================ SLIDE 15 - FECHO (escuro) ===================
(() => {
  const s = pptx.addSlide();
  s.background = { color: C.dbg };
  s.addShape("rect", { x: 0, y: 0, w: W, h: 0.16, fill: { color: C.gold } });
